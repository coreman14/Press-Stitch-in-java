...
public class press_stitch {
	...
	//-----------------------------------------------------------------------------
	public static void printRed(String s) {print("\033[1;31m" + s + "\033[0m"); new Exception().printStackTrace();}

	//-----------------------------------------------------------------------------
	public static void showError(String txt) {printRed("Error: " + txt);}
...
//	    print("Variable '" + pyVar + "' becomes '" + pyVal + "'");
	  ...
	  else {flagError(rpFile, lineNum, "Unsupported operator '" + fields[1] + "', line is: " + line);}
	}
	//-----------------------------------------------------------------------------
	public boolean calculateCondition(RenPyThread thread, int lineNum, String[] fields) {
	    ...
	    while(offset < fields.length) {
	        ...
	        if (!thread.vars.containsKey(varname)) {return false;}
	        if (condition.equals("==")) {
	            cont = false;
	            ...
	            if (thread.vars.get(varname).equals(RenPyFile.trimStringByStringMulti(value, "\"", "'"))) {return true;}
	            if (cont) {
	                offset = offset + 1;
	                value = fields[offset + 2];
	                if (thread.vars.get(varname).equals(RenPyFile.trimStringByStringMulti(value, "\"", "'"))) {return true;}
	            ...
	            if (Integer.parseInt(thread.vars.get(varname)) >= Integer.parseInt(RenPyFile.trimStringByStringMulti(value, "\"", "'"))) {
	                return true;
	            }
	        }
	        else {
	        ...
	        if ((offset < fields.length && !fields[offset].equals("or"))) {
	            showError(lineNum + ": Boolean operator " +
	                      fields[offset] + " not supported, fields are [" + String.join(",", fields) + "]");
	            System.exit(1);
	        }
	        offset = offset + 1;
	    }
	...
	public void processIfStep(RenPyFile rpFile, RenPyThread thread) {
	    ...
	    if((fields[0].equals("if")) || (fields[0].equals("elif"))) {
	        ...
	        if (condition && !obj2.hasExecuted){
	            ...
	            thread.stack.add(new RenPyBlock(
	                obj.lineNum + 1, obj.indent + 4));
	        }
	        ...
	        if (getIndentOf(line) == indent) {
	            ...
	            if (!((menuItem.charAt(0) == '#') || menuItem.startsWith("\"{s}"))) {
	                ...
	                if (!(condition.equals(":"))) {
	                    ...
	                    res = calculateCondition(
	                        thread, lineNum, condition.trim().split("\\s+"));
	                }
	                if (res) {
	                	
	                    RenPyThread newThread = thread.clone();
	                    newThread.stack.add(new 
	                        RenPyBlock(lineNum + 1, indent + 4));
	...
	public String processShow(RenPyFile rpFile, RenPyThread thread, int lineNum) {
	    ...
	    String base = "";
	    int i = 2;
	    ArrayList<String> placement = new ArrayList<String>(Arrays.asList(new String[]{"as", "at", "behind", "with", "zorder"}));
	    while (i < fields.length) {
	    ...
	    if (!hasMapped) {
	        ...
	        return(flagError(rpFile, lineNum, "Mapping failed, source file '" + exFile + "' not found. Line being processed is: " + String.join(",", fields)));
	    }
	    if (mappedFile.equals("")){
	        return(flagError(rpFile, lineNum, "Mapping failed, source file '" + exFile + "' exists but has no mapping. Line being processed is: " + String.join(",", fields)));
	    }
	    // Map V6 if present
...
//	//-----------------------------------------------------------------------------
	  ...
	  while (labelCalls.size() > 0 || (threads.size() > 0)){
	    ...
	    while(threads.size() > 0) {
	      ...
	      if ((threads.size() % 10) == 0) {
	        print("[Depth " + iterations + "] Paths: " + duplicates + " dupe, " + numThreads + " total, " + threads.size() + " left this depth");
	        }
	      }
	    iterations += 1;
	  }
	...
	public void main(String[] args) {
		...
		boolean doScan  = true;
		boolean doV6    = false;
		ArrayList<String> argsChecks = new ArrayList<String>(Arrays.asList(new String[]{"clean","inlineerrors","nociel","noeliza","nogoopy","noscan","v6"}));
		if (args.length > 0) {
		  ...
		  if (doV6) {
		    ...
		    dayzero.findLabels();
		    dayzero.findShows();
		    addLabelCall(dayzero, "GameStart", new RenPyThread((LinkedHashMap<String, String>)pyVariables.clone(), new ArrayList<RenPyObject>()));
		  ...
		  if (doCiel) {
		    ...
		    RenPyFile cielPath = new RenPyFile("C", backgrounds_map.backgroundMap35, characterImages.characterImageMap35, v6map);
		    ...
		    addLabelCall(cielPath, "leftit", new RenPyThread(new LinkedHashMap<String, String>(), new ArrayList<RenPyObject>()));
		  ...
		  if (doEliza) {
		    ...
		    RenPyFile elizaPath = new RenPyFile("E", backgrounds_map.backgroundMap45, characterImages.characterImageMap45, v6map);
		    ...
		    if (doScan) {
		      ...
		      addLabelCall(elizaPath, "eliza", new RenPyThread((LinkedHashMap<String, String>)pyVariables.clone(), new ArrayList<RenPyObject>()));
		  ...
		  if (doGoopy) {
		    ...
		    RenPyFile goopyPath = new RenPyFile("G", backgrounds_map.backgroundMap35, characterImages.characterImageMap35, v6map);
		    ...
		    // Process the path
		    addLabelCall(goopyPath, "elizagoopypath", new RenPyThread(new LinkedHashMap<String, String>(), new ArrayList<RenPyObject>()));
